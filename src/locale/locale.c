...
#define SET_PROPERTY(name)                                                     \
  if (value_len == 0) {                                                        \
    zend_update_property_null(ecma_intl_ce_Locale, object, name,               \
                              sizeof(name) - 1);                               \
  } else {                                                                     \
    zend_update_property_stringl(ecma_intl_ce_Locale, object, name,            \
                                 sizeof(name) - 1, value, value_len);          \
  }
...
static void set_calendar(zend_object *object, char *locale_id) {
  ...
  value_len = uloc_getKeywordValue(locale_id, KEYWORD_ICU_CALENDAR, value,
                                   ULOC_FULLNAME_CAPACITY, &status);

  if (value_len == 0) {
    zend_update_property_null(ecma_intl_ce_Locale, object, PROPERTY_CALENDAR,
                              sizeof(PROPERTY_CALENDAR) - 1);
  } else {
    zend_update_property_string(
        ecma_intl_ce_Locale, object, PROPERTY_CALENDAR,
        sizeof(PROPERTY_CALENDAR) - 1,
...
static void set_calendars(zend_object *object, char *locale_id) {
  ...
  values = ucal_getKeywordValuesForLocale(KEYWORD_ICU_CALENDAR, locale_id, 1,
                                          &status);
  ...
  if (U_FAILURE(status)) {
    ...
    zend_update_property(ecma_intl_ce_Locale, object, PROPERTY_CALENDARS,
...
static void set_case_first(zend_object *object, char *locale_id) {
  ...
  value = (char *)ecalloc(1, sizeof(char *) * ULOC_FULLNAME_CAPACITY);
  value_len = uloc_getKeywordValue(locale_id, KEYWORD_ICU_CASE_FIRST, value,
                                   ULOC_FULLNAME_CAPACITY, &status);
...
static void set_collation(zend_object *object, char *locale_id) {
  ...
  value = (char *)ecalloc(1, sizeof(char *) * ULOC_FULLNAME_CAPACITY);
  value_len = uloc_getKeywordValue(locale_id, KEYWORD_ICU_COLLATION, value,
                                   ULOC_FULLNAME_CAPACITY, &status);

  if (value_len == 0) {
    zend_update_property_null(ecma_intl_ce_Locale, object, PROPERTY_COLLATION,
                              sizeof(PROPERTY_COLLATION) - 1);
  } else {
    zend_update_property_string(
        ecma_intl_ce_Locale, object, PROPERTY_COLLATION,
        sizeof(PROPERTY_COLLATION) - 1,
...
static void set_hour_cycle(zend_object *object, char *locale_id) {
  ...
  value = (char *)ecalloc(1, sizeof(char *) * ULOC_FULLNAME_CAPACITY);
  value_len = uloc_getKeywordValue(locale_id, KEYWORD_ICU_HOUR_CYCLE, value,
                                   ULOC_FULLNAME_CAPACITY, &status);
...
static void set_hour_cycles(zend_object *object, char *locale_id) {
  ...
  if (U_FAILURE(status)) {
    ...
    zend_update_property(ecma_intl_ce_Locale, object, PROPERTY_HOUR_CYCLES,
                         sizeof(PROPERTY_HOUR_CYCLES) - 1, &hour_cycles);
...
static void set_language(zend_object *object, char *locale_id) {
  ...
  value = (char *)ecalloc(1, sizeof(char *) * ULOC_FULLNAME_CAPACITY);
  value_len =
      uloc_getLanguage(locale_id, value, ULOC_FULLNAME_CAPACITY, &status);
...
static void set_numbering_system(zend_object *object, char *locale_id) {
  ...
  value = (char *)ecalloc(1, sizeof(char *) * ULOC_FULLNAME_CAPACITY);
  value_len = uloc_getKeywordValue(locale_id, KEYWORD_ICU_NUMBERING_SYSTEM,
                                   value, ULOC_FULLNAME_CAPACITY, &status);

  if (value_len == 0) {
    zend_update_property_null(ecma_intl_ce_Locale, object,
                              PROPERTY_NUMBERING_SYSTEM,
                              sizeof(PROPERTY_NUMBERING_SYSTEM) - 1);
  } else {
    zend_update_property_string(
        ecma_intl_ce_Locale, object, PROPERTY_NUMBERING_SYSTEM,
        sizeof(PROPERTY_NUMBERING_SYSTEM) - 1,
...
static void set_numeric(zend_object *object, char *locale_id) {
  ...
  value = (char *)ecalloc(1, sizeof(char *) * ULOC_FULLNAME_CAPACITY);
  uloc_getKeywordValue(locale_id, KEYWORD_ICU_NUMERIC, value,
                       ULOC_FULLNAME_CAPACITY, &status);

  if (strcmp(value, "yes") == 0) {
    zend_update_property_bool(ecma_intl_ce_Locale, object, PROPERTY_NUMERIC,
...
static void set_region(zend_object *object, char *locale_id) {
  ...
  value = (char *)ecalloc(1, sizeof(char *) * ULOC_FULLNAME_CAPACITY);
  value_len =
      uloc_getCountry(locale_id, value, ULOC_FULLNAME_CAPACITY, &status);
...
PHP_METHOD(Ecma_Intl_Locale, __construct) {
  ...
  if (options) {
    char *constructed_locale =
        (char *)ecalloc(1, sizeof(char *) * ULOC_FULLNAME_CAPACITY);
    if (ecma_intl_build_locale(constructed_locale, tag,
                               Z_ECMA_LOCALE_OPTIONS_P(options))) {
      bcp47_locale_len = ecma_intl_toCanonicalBcp47LanguageTag(
          constructed_locale, bcp47_locale);
      uloc_canonicalize(constructed_locale, icu_locale, ULOC_FULLNAME_CAPACITY,
                        &status);
    }
...
void ecma_intl_register_Locale() {
  ...
  memcpy(&ecma_intl_locale_obj_handlers, &std_object_handlers,
