...
#define UPDATE_BOOL_PROPERTY(property, var)                                    \
  zend_update_property_bool(ecma_intl_ce_Locale_Options, &options->std,        \
...
static zend_object *
ecma_intl_locale_options_obj_new(zend_class_entry *class_type);
static void ecma_intl_locale_options_obj_free(zend_object *object);

static zend_object *
ecma_intl_locale_options_obj_new(zend_class_entry *class_type) {
  ecma_intl_locale_options_obj *intern =
      zend_object_alloc(sizeof(ecma_intl_locale_options_obj), class_type);
...
void ecma_intl_locale_options_obj_free(zend_object *object) {
  ecma_intl_locale_options_obj *options_obj =
      ecma_intl_locale_options_obj_from_obj(object);
...
PHP_METHOD(Ecma_Intl_Locale_Options, __construct) {
  ...
  if (numbering_system) {
    UPDATE_STRING_PROPERTY(PROPERTY_NUMBERING_SYSTEM, numbering_system,
...
void ecma_intl_register_Locale_Options() {
  ...
  memcpy(&ecma_intl_locale_options_obj_handlers, &std_object_handlers,
         sizeof(zend_object_handlers));
  ecma_intl_locale_options_obj_handlers.offset =
      XtOffsetOf(ecma_intl_locale_options_obj, std);
  ecma_intl_locale_options_obj_handlers.free_obj =
      ecma_intl_locale_options_obj_free;
}
